...
channel_id = next((channel['id'] for channel in sc.api_call("channels.list")['channels'] if channel['name'] == channel_name), None)
...
all_events = sc.api_call("channels.history",
                         channel=channel_id,
                         count=1000)
# only_leaves = [event for event in all_events['messages'] if event['sub_type'] == 'channel_leave']
only_leaves = []
...
only_leaves_in_period = set([leave['user'] for leave in only_leaves if from_timestamp < float(leave['ts']) < to_timestamp])
